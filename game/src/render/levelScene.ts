...
// 잠수부 그리기 — 예전 diverRig.ts 를 여기로 옮겨왔다. 3D 로 넘어가며 그 파일은 지웠고
// (game/src/render3d/diver.ts 가 대신한다), 이 2D 장면(LevelScene)은 이제 어디서도
// 생성하지 않아 죽은 코드지만 8번 과제가 되살리거나 지울지 정할 때까지 남겨둔다.
// 그때까지는 이 파일이 diverRig.ts 의 유일한 남은 소비자이므로 그 내용을 그대로
// 이 파일 전용으로 들여왔다 — 동작은 한 글자도 바꾸지 않았다.
//
...
//
// 지금은 **포즈를 미리 잡아 구운 한 장**을 쓴다. 움직임은 관절이 아니라 **몸 전체**가
// 만든다: 물에 뜬 사람은 팔다리를 휘젓는 게 아니라 천천히 오르내리며 좌우로 기운다.

/** 머리 꼭대기 ~ 발바닥 전체 높이 (배율 계산용) */
const DIVER_SPAN = DIVER_META.rig.span;
 ...
 */
const DIVER_FEET = DIVER_META.rig.feet;
 ...
 */
function drawDiver(ctx: CanvasRenderingContext2D, pose: DiverPose): void {
...
/** 기포가 나오는 지점 (헬멧 옆) */
function helmetPos(pose: DiverPose): [number, number] {
