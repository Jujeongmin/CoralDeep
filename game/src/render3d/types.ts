...
export interface Stage {
  ...
  rescued(): void;
  /**
   * 산소가 다 떨어져 포식자에게 잡혔을 때. 한 판에 한 번만 부른다.
   *
   * 실패 모달이 뜨기 전(screens/level.ts 가 1.1초를 준다)에 포식자가 덮치고 잠수부가
   * 물려 가는 것을 보여준다 — 이게 없으면 산소 막대가 0 이 된 순간 화면이 그대로
   * 멈춰 있다가 실패 모달만 떠서, 무엇 때문에 졌는지가 화면에 안 남는다.
   */
  devour(): void;
  dispose(): void;
}
