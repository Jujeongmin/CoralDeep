...
**Interfaces:**
- Consumes: `tools/lib/fbx.mjs` (기존 FBX 파서), `tools/bake-diver-3d.mjs` (좌표계 보정·팔 내리기 코드)
- Produces:
  - `diver.glb` — 위치·법선·버텍스 컬러를 가진 단일 메시. Y-up, 발이 `y=0`, 정수리가 `y=1`, +Z 정면
...
// 이제 런타임이 3D 라서 기하를 그대로 내보낸다. 좌표계 보정과 팔 내리기는 그 도구의
// 것을 그대로 쓴다 — 두 번 틀렸던 곳이라 다시 유도하지 않는다:
//   * Blender 로 내보낸 FBX 는 Z-up 이다 (FBX 표준의 Y-up 이 아니다).
//     틀리면 정수리에서 내려다본 그림(팔이 좌우로 뻗은 T 자)이 나온다.
//   * Blender 인물은 -Y 를 향해 서 있다. 그대로 보면 뒤통수만 나온다.
//
// 옷은 다시 칠한다. 원본은 흰 셔츠 + 청바지라 그대로 두면 심해 잠수부로 안 읽힌다.
// 구리 헬멧이 정체를 한 번에 알리고, 네오프렌 몸통과 납 부츠가 그걸 받친다.
// 헬멧이 원래 불투명 판이라 얼굴이 없는데, 구리로 칠하면 그게 결함이 아니라 면갑으로
// 보인다 — 공포는 표정이 아니라 흔들림의 폭으로 전한다.
//
...
import { parseFbx } from './lib/fbx.mjs';

const ROOT = join(dirname(fileURLToPath(import.meta.url)), '..');
const SRC = join(ROOT, 'assets-raw/quaternius-men/SpaceSuit.fbx');
const OUT = join(ROOT, 'game/src/assets/sprites3d/diver.glb');

// 높이 비율로 부위를 나눠 색을 준다 (0 = 발, 1 = 정수리).
// 몸통을 더 어둡게 두면 640m 물빛에 실루엣이 잠긴다. 중간 톤을 유지하고 대비는
// 헬멧이 만들게 한다.
const PALETTE = [
  { upTo: 0.14, rgb: [0.22, 0.24, 0.28] }, // 납 부츠
  { upTo: 0.52, rgb: [0.13, 0.2, 0.26] },  // 네오프렌 다리
  { upTo: 0.8, rgb: [0.16, 0.26, 0.33] },  // 몸통
  { upTo: 1.01, rgb: [0.72, 0.45, 0.18] }, // 구리 헬멧
];

function colorFor(h) {
  for (const p of PALETTE) if (h <= p.upTo) return p.rgb;
...
`tools/bake-diver-3d.mjs` 를 읽고 다음 블록을 그대로 가져온다. **새로 유도하지 않는다** — 그 파일이 이미 맞는 값을 갖고 있다.

1. `parseFbx(readFileSync(SRC))` 로 정점·인덱스를 읽는다
2. Z-up → Y-up 회전
3. -Y 정면 → +Z 정면 회전
4. 어깨를 축으로 팔 정점 75° 내리기 (T 포즈라 팔이 X 축에 정렬돼 있다)

그 뒤에 이 파일에서 새로 하는 것:

```js
// 발이 y=0, 정수리가 y=1 이 되도록 정규화한다.
// 런타임이 칸 크기에 맞춰 scale 만 곱하면 되게 하려면 높이가 1 이어야 한다.
function normalize(position) {
  ...
  for (let i = 0; i < position.length; i += 3) {
    ...
    position[i + 1] = (position[i + 1] - minY) * s;
    position[i + 2] = (position[i + 2] - cz) * s;
  }
}

// 면 법선을 정점에 누적한 뒤 정규화한다. flatShading 을 쓸 것이므로 정밀할 필요는 없다.
...
function paint(position) {
  ...
  for (let i = 0; i < position.length; i += 3) {
    const rgb = colorFor(position[i + 1]);
    color[i] = rgb[0];
    color[i + 1] = rgb[1];
    color[i + 2] = rgb[2];
  }
...
function writeGlb(position, normal, color, index) {
  ...
  const total =
    12 + 8 + jsonBuf.length + jsonPad.length + 8 + bin.length + binPad.length;
...
그리고 맨 아래에서 순서대로 부른다: 읽기 → 좌표계 보정 → 팔 내리기 → `normalize` → `computeNormals` → `paint` → `writeGlb`.
...
Expected: `diver.glb` 생성, 정점 수 출력. 정점이 5,000 을 넘으면 저폴리 원본이 아니므로 소스를 다시 확인한다.
...
export class Predators {
  ...
  constructor(
    ...
    this.body = new THREE.Mesh(new THREE.TubeGeometry(this.curve, 24, 0.28, 6, false), this.mat);
  ...
  private rebuild(): void {
    ...
    const next = new THREE.TubeGeometry(this.curve, 24, 0.28 + this.danger * 0.1, 6, false);
    this.body.geometry.dispose();
    this.body.geometry = next;
